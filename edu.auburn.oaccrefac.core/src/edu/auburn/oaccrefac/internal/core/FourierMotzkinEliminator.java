 ...
 * 
 * 
 *       We assume that every inequality in the matrix is of the form (Ax <= b)
 *       Determines whether a real solution to a system of linear inequalities
...
public class FourierMotzkinEliminator {
     ...
     * 
     * @param matrixIn
     *            system of linear inequalities on which to find whether a real
     ...
     * 
     * @param matrixIn
     *            system of linear inequalities on which to find whether a
     *            integer solution exists or not
     * @return whether a solution exists or not
     */
    public boolean eliminateForIntegerSolutions(Matrix matrixIn) {
        ...
        if (matrixIn.getNumColumns() < 8) {
            if (!eliminateForRealSolutions(matrixIn.cloneMatrix()))
                return false;
        }
        ...
        // should only be simple inequalities - all coefficients should be 0
            ...
            for (int i = 0; i < matrixIn.getNumRows(); i++) {
                ...
                // Ax <= b ... b must be 0 or greater
                if (matrixIn.getValueAtMatrixIndex(i, numColumns) < 0
                        && matrixIn.getValueAtMatrixIndex(i, eliminatedVar - 1) == 0.0) {
                    return false;
                }
            }
        }
     ...
     * @return the matrix produced after doing a single real projection on
     *         matrixIn
     */
    public Matrix realProjection(Matrix matrixIn, int eliminatingVar) {
        ...
        if (lowerBoundSet.isEmpty() || upperBoundSet.isEmpty()) {
            matrixIn = deleteRowsInBoundingSet(matrixIn, combineBoundingSets(lowerBoundSet, upperBoundSet));
            return matrixIn;
        }
        ...
        matrixIn = deleteRowsInBoundingSet(matrixIn, combineBoundingSets(lowerBoundSet, upperBoundSet));
        matrixIn.trimRowsToSize(); // helps when growth of inequalities tends
                                   // toward exponential
        return matrixIn;
    }
     ...
     * @param eliminatingVar
     *            the variable being eliminated
     * @return the matrix produced after doing a single inexact integer
     *         projection on matrixIn
     */
     ...
     * @param eliminatingVar
     *            the variable being eliminated
     * @return the matrix produced after doing a single dark shadow projection
     *         on matrixIn
     */
    public Matrix darkShadowIntegerProjection(Matrix matrixIn, int eliminatingVar) {
        ...
        for (int i = 0; i < lowerBoundSet.size(); i++) {
            for (int k = 0; k < upperBoundSet.size(); k++) {
                double[] newRow = calculateDarkShadow(matrixIn, lowerBoundSet.get(i), upperBoundSet.get(k),
                        eliminatingVar);
                matrixIn.addRowAtIndex(matrixIn.getNumRows(), newRow);
            }
        }
     ...
     * @return matrixIn minus any redundant rows redundant meaning they are the
     *         same inequality
     */
     ...
     * @param matrixIn
     *            matrix to be changed
     * @return matrixIn minus any inconsistent inequalities inconsistent meaning
     *         Aj <= bj and Ai <= bk where Aj = -Ak and -bk > bj
     */
    ...
    /**
     * @param matrixIn
     * @return whether the matrix contains inconsistent inequalities or not
     *         inconsistent meaning Aj <= bj and Ai <= bk where Aj = -Ak and -bk
     *         > bj
     */
     ...
     * @return whether the two arrays create an inconsistency inconsistent
     *         meaning Aj <= bj and Ai <= bk where Aj = -Ak and -bk > bj
     */
    ...
    /**
     * TODO don't need Sorts matrixIn based on the contents of the bounding sets
     * Puts any rows belonging to either of the sets at the top of the matrix
     * 
     * @param matrixIn
     *            matrix to be sorted
     * @param lowerBoundSet
     ...
     * with sortMatrixByBoundingSetContents to delete bounding sets U and L
     * 
     * @param matrixIn
     * @param size
     ...
     * @return a list of integers that contain the row indices for a lower
     *         bounding set L = { i | a_ij < 0 }
     */
     ...
     * @param matrixIn
     * @param colIndex
     * @return a list of integers that contain the row indices for an upper
     *         bounding set U = { i | a_ij > 0 }
     */
     ...
     * @param matrixIn
     *            matrix to be changed
     * @return unconstrained an array describing whether a variable is
     *         constrained
     */
    public boolean[] determineUnconstrainedVariables(Matrix matrixIn) {
        ...
        for (int i = 0; i < numOfVariables; i++) {
            if (calculateLowerBoundSet(matrixIn, i).isEmpty() || calculateUpperBoundSet(matrixIn, i).isEmpty())
     ...
     * @return matrixIn minus any row containing a non-zero value in the column
     *         "unconstrainedIndex"
     */
    ...
    public double[] arraySubtraction(double[] biggerArray, double[] smallerArray) {
        for (int i = 0; i < biggerArray.length; i++) {
            biggerArray[i] = biggerArray[i] - smallerArray[i]; // possible
                                                               // over/underflow
                                                               // here. unlikely
        }
        return biggerArray;
     ...
     * @param solutionSpace
     * @param elimVar
     * @return whether a given variable in solutionSpace potentially has a
     *         solution. Only returns true if it definitely does not have a
     *         solution. False does not mean that no solutions exist. Other
     *         variables could affect it and are not taken into account here.
     */
    public boolean isSolutionSetEmpty(Matrix solutionSpace, int elimVar) {
        ...
        for (int i = 0; i < numberOfEquations; i++) {
            ...
            if (multipleNonZeroValuesExistInArrayExcludingLastPlace(equation))
                ...
                if (equation[elimVar] != 0)
                    lastNum = lastNum / equation[elimVar];
    ...
    /**
     * @param arrayIn
     * @return whether arrayIn contains multiple non-zero values (exluding last
     *         column)
     */
     ...
     * @param matrixIn
     * @param colNum
     * @return rowsWithVar a matrix with all rows from matrixIn where the value
     *         at colNum was non-zero
     */
     ...
     * @param matrixIn
     * @param elimVar
     *            the variable to determine if inexact projection
    ...
    /**
     * Determines the dark shadow on two rows in regards to the eliminating
     * variable
     * 
     * @param matrixIn
     * @param lowerBoundSetVal
     *            a row index in matrixIn
     * @param upperBoundSetVal
     *            a row index in matrixIn
     * @param elimVar
     ...
     * @param matrixIn
     * @param combinedSet
     *            the combined set of the lowerbound and upperbound sets (comes
     *            from combineBoundingSets)
     ...
     * @param lowerBoundSet
     * @param upperBoundSet
     * @return a List containing the values from both lowerBoundSet and
     *         uppserBoundSet in sorted order
     */
