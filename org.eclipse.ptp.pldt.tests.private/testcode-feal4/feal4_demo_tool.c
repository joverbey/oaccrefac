...
uint64_t combineHalves(uint32_t leftHalf, uint32_t rightHalf) {
	return (((uint64_t) (leftHalf)) << 32LL)
			| (((uint64_t) (rightHalf)) & 0xFFFFFFFFLL);
}
...
uint32_t crackLastRound(uint32_t outdiff) {
	...
	printf("\tCracking...");
	uint32_t finalK = 0;
	BOOL setFinalK = FALSE;
#pragma acc data copyin(cipher0,cipher1,numPlain,outdiff) copy(finalK)
	{
		for (uint32_t i = 0; i < 0xFFFFFFFFL && !setFinalK; i += 65535) {
#pragma acc parallel loop copyin(i) copy(setFinalK)
...
uint32_t round2(uint64_t inputDiff, uint32_t outDiff, uint32_t crackedSubkey3,
...
int32_t main() {
	...
	uint32_t crackedSubkey1 = round2(inputDiff3, outDiff, crackedSubkey3,
			crackedSubkey2);
	printf("ROUND 1\n");
	...
	uint32_t startTime = time(NULL);
	BOOL setGuess = FALSE;
#pragma acc data copyin(cipher0,numPlain,plain0) copy(crackedSubkey0,crackedSubkey4,crackedSubkey5)
	{
		for (uint32_t i = 0; i < 0xFFFFFFFFL && !setGuess; i += 65535) {
#pragma acc parallel loop copyin(i) copy(setGuess)
			for (uint32_t j = 0; j < 65535; j++) {
				uint32_t guessK0 = i + j;
				uint32_t guessK4 = 0;
				uint32_t guessK5 = 0;
#pragma acc loop seq
				for (int32_t c = 0; c < numPlain; c++) {
					...
					uint32_t cipherLeft0 = leftHalf(cipher0[c]);
					uint32_t cipherRight0 = rightHalf(cipher0[c]);
					uint32_t tempy0 = fBox(cipherRight0 ^ guessK0)
							^ cipherLeft0;
					if (guessK4 == 0) {
...
#pragma acc atomic write
	...
	printf("found subkeys : 0x%08lx  0x%08lx  0x%08lx\n", crackedSubkey0,
			crackedSubkey4, crackedSubkey5);
	printf("\tTime to crack round #1 = %i seconds\n", (endTime - startTime));
	printf("\n\n");
	uint32_t crackedSubkeys[6] = { crackedSubkey0, crackedSubkey1,
			crackedSubkey2, crackedSubkey3, crackedSubkey4, crackedSubkey5 };
