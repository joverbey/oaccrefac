...
//
// This file contains the parallelized code with a fixed seed which is totally
// successful.
...
uint32_t crackLastRound(uint32_t outdiff) {
  ...
  uint32_t per = 0xFFFFL;
  #pragma acc data copyin(cipher1, cipher0)
  for (uint32_t i = 0; i < 0xFFFFFFFFL/per && !setFinalK; i++) {
    #pragma acc parallel loop
    for (uint32_t fakeK = i*per; fakeK < (i+1)*per; fakeK++) {
      int32_t score = 0;
      #pragma acc loop seq
      for (int32_t c = 0; c < numPlain; c++) {
...
int32_t main() {
  ...
  uint32_t per = 0xFFFFL;
  bool setGuess = false;
  #pragma acc data copyin(cipher1, cipher0, plain0)
  for (uint32_t i = 0; i < 0xFFFFFFFFL/per && !setGuess; i++) {
    #pragma acc parallel loop
    for (uint32_t guessK0 = i*per; guessK0 < (i+1)*per; guessK0++) {
      uint32_t guessK4 = 0;
      uint32_t guessK5 = 0;
      #pragma acc loop seq
      for (int32_t c = 0; c < numPlain; c++) {
