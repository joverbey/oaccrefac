...
public abstract class RefactoringTestComplete<R extends CRefactoring> extends RefactoringTest<R> {
    @Parameters(name = "{0}")
    ...
    protected RefactoringTestComplete(Class<R> refactoringClass, File fileContainingMarker, int markerOffset,
    ...
    protected void compareAgainstResultFile(String originalSource, TextSelection selection) 
        ...
        for (String filename : files.keySet()) {
            ...
            expected = injectResult(originalSource, selection, toInject);
            actual = readWorkspaceFile(filename);
        }
 //$NON-NLS-1$ //$NON-NLS-2$
